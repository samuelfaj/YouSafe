...
class db{
    public $default_connection = array(
        ...
        'database' => 'yousafe'     ,  // string - Default Database name.
...
switch ($_GET['act']){
    case 'insert':
        $_POST['data_registro'] = date('Y-m-d H:i:s',strtotime(str_replace('/','-',$_POST['data_registro'])));
        $sql->insert($_POST);
    break;
    default:
...
echo json_encode(array('result' => $sql->query->result,'return'=>$sql->query->fetch));